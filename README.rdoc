...
3. For authorizable objects that need to have a role maintained for the user 
   that owns them for a specific relationship, use the always_has class method made
   available by acts_as_authorizable.
...
* The ability to have a predetermined role with an authorizable instance 
  represented by an existing belongs_to relationship kept up to date for you.
...
If you have a relationship with the User model on an authorizable model 
that you would like kept up to date, use always_has as below, where an 'owner'
role will always be kept current for a given Presentation instance:

  class Presentation

    belongs_to :user
    always_has :owner, :as => :user   # always_has would assume a 
                                      # relationship called 'owner' if
                                      # you were to omit ':as => :user'
